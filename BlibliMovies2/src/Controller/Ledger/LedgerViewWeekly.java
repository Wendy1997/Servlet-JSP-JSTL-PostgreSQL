...
public class LedgerViewWeekly extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try{
            ...
            List<Double> pageCounter = invoiceService.getCountWeeklyInvoice(request.getParameter("date"), (int)request.getSession().getAttribute(storeIdSession));

            // Pengambilan list invoice minggu tersebut
            List<Invoice> invoiceList = invoiceService.getWeeklyInvoice(request.getParameter("date"), (int)request.getSession().getAttribute(storeIdSession), initialPage);
